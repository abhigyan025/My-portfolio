...
/* ===== READING MODE (shared by essays + short stories) ===== */
...
/* ===== SHARED: build a "featured + 3 + show more" content type (essays/stories) ===== */
function renderContentList({
  items,
  gridSelector,
  featuredWrapSelector,
  featuredLabel,
  featuredBtnLabel,
  idPrefix,
  showMoreId
  ...
  const featuredWrap = document.querySelector(featuredWrapSelector);
  if(!grid || !readingContent) return;

  // clear any previously rendered full-view panels for this content type
  document.querySelectorAll(`.essay-full[data-prefix="${idPrefix}"]`).forEach(el => el.remove());

  if(!items.length){
    grid.innerHTML = `<p style="color:var(--slate)">Nothing here yet.</p>`;
  ...
  function makeFullPanel(item, id){
    const words = (item.body||"").split(" ").length;
    const mins = Math.max(1, Math.round(words/200));
    const full = document.createElement("div");
    full.id = id;
    full.className = "essay-full";
    full.dataset.prefix = idPrefix;
    full.innerHTML = `
      <span class="backToEssays">&larr; Back</span>
      <p class="essay-meta"><span class="read-time">${mins} min read</span></p>
      <h2>${item.title||"Untitled"}</h2>
      ${(item.body||"").split("\n").filter(Boolean).map(p=>`<p>${p}</p>`).join("")}
      <div class="share-row">
        <a class="btn secondary" target="_blank" rel="noreferrer" href="https://wa.me/?text=${encodeURIComponent(item.title||'')}">Share on WhatsApp</a>
        <a class="btn secondary" target="_blank" rel="noreferrer" href="https://twitter.com/intent/tweet?text=${encodeURIComponent(item.title||'')}">Share on X</a>
      </div>`;
  ...
  // featured block
  if(featuredWrap){
    const fid = `${idPrefix}-featured`;
    featuredWrap.innerHTML = `
      <span class="read-time">${featuredLabel}</span>
      <h3>${featured.title || "Untitled"}</h3>
      <p>${featured.excerpt || ""}</p>
      <div style="margin-top:24px">
        <button class="btn primary essayBtn" type="button" data-target="${fid}">${featuredBtnLabel}</button>
      </div>`;
    makeFullPanel(featured, fid);
  }

  // grid of next 3 (+ hidden ones, revealed via Show More)
  grid.innerHTML = "";
  function cardHtml(item, id){
    const words = (item.body||"").split(" ").length;
    const mins = Math.max(1, Math.round(words/200));
    return `
  ...
  visible.forEach((item, i) => {
    const id = `${idPrefix}-${i}`;
    grid.insertAdjacentHTML("beforeend", cardHtml(item, id));
    makeFullPanel(item, id);
  });

  if(hidden.length){
    const moreId = showMoreId;
    grid.insertAdjacentHTML("beforeend", `<div class="show-more-wrap"><button type="button" class="btn secondary" id="${moreId}">Show More</button></div>`);
    document.getElementById(moreId).addEventListener("click", (e) => {
      hidden.forEach((item, i) => {
        const id = `${idPrefix}-more-${i}`;
        makeFullPanel(item, id);
      });
      const wrap = e.target.closest(".show-more-wrap");
      const cardsHtml = hidden.map((item, i) => cardHtml(item, `${idPrefix}-more-${i}`)).join("");
      wrap.insertAdjacentHTML("beforebegin", cardsHtml);
      wrap.remove();
    });
  }
}
